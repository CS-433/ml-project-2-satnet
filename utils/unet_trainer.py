...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        val_loss = np.mean(metrics["val_loss"][-len(val_dataloader):])
        print("-" * 30)
        print(f"Train Loss EPOCH {epoch + 1}: {train_loss:.4f}")
        print(f"Valid Loss EPOCH {epoch + 1}: {val_loss:.4f}")
        print(f"Train Accuracy EPOCH {epoch + 1}: {np.mean(metrics['train_accuracy'][-len(train_dataloader):]):.4f}")
        print(f"Valid Accuracy EPOCH {epoch + 1}: {np.mean(metrics['val_accuracy'][-len(val_dataloader):]):.4f}")
        print("-" * 30)
