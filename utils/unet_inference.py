...
def single_image_inference(image_pth, model_pth, device, output_dir, output_name):
    model = UNet(in_channels=3, out_channels=1).to(device)
    model.load_state_dict(torch.load(model_pth, map_location=torch.device(device)))
...
if __name__ == "__main__":
    ...
    single_image_inference(SINGLE_IMG_PATH, MODEL_PATH, device, OUTPUT_DIR, "single_predicted_2.png")
    print("Done")